        ...
        const char *engineName) : _window(window)
{
    ...
    Assert(vkCreateGraphicsPipelines(_device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &_graphicsPipeline) ==
           VK_SUCCESS,
    ...
    Assert(vkCreateGraphicsPipelines(_device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &_graphicsPipelineWireframe) ==
           VK_SUCCESS,
           "Failed to create graphics pipeline!");
...
void VulkanRenderer::render(ImDrawData *drawData)
{
    ...
    uint32_t imageIndex;
    VkResult result = vkAcquireNextImageKHR(_device, _swapchain, (u64) -1, _imageAvailableSemaphores[_currentFrame],
...
void VulkanRenderer::updateUniformBuffer(u32 currentFrame)
{
    ...
    ubo.proj = glm::perspective(glm::radians(45.0f), (f32) _swapchainExtent.width / (f32) _swapchainExtent.height, 0.1f,
                                10.0f);
...
void
VulkanRenderer::createCommandBuffers(VkCommandBuffer *commandBuffer, u32 commandBufferCount, VkCommandPool &commandPool)
{
        ...
        u32 imageIndex)
{
    ...
    vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                      wireframe ? _graphicsPipelineWireframe : _graphicsPipeline);
